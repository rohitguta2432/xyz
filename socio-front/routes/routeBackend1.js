...
var twitter = new twitterAPI({
	consumerKey: 'ElTI9BzpiH4W9Th9lpDNLHux6',
	consumerSecret: 'mtBLyOvdoIW7kd8JxKmijKfPleFILiPhXsSvm0KHV6cuIff3bk',
...
router.route('/twitter/trends').get(function (req, res) {
	...
	try {
		...
		twitter.trends('place', _prm, '855706086405439488-xdOjZRtbtfyLfNUMseD6cfCKU1OGmi4', 'VqjtNumrsj9WVwWMbqgjMCDzsYDkFGX1cAJoslafDRFqR', function (error, _data, response) {
			if (error) {
			} else {
				var _response = {
					status: 200,
...
router.route('/twitter/trendingPost').get(function (req, res) {
	var reqData = req.query;
	try {
		twitter.search(reqData, '855706086405439488-xdOjZRtbtfyLfNUMseD6cfCKU1OGmi4', 'VqjtNumrsj9WVwWMbqgjMCDzsYDkFGX1cAJoslafDRFqR', function (error, _data, response) {
			if (error) {
			} else {

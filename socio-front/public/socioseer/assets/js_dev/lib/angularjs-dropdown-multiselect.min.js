			...
			*/
			...
			function dropdownMultiselectController($scope, $element, $filter, $document) {
				...
				function toggleDropdown(that) {
					/*
					Edited by Gaurav/Anjani to prevent multiple dropdown open/close
					start
					**/
					if (!search('id', arrayLoop)) {
						arrayLoop.push({
							id: that.$id,
							element: that
						});
					...
					angular.forEach(arrayLoop, function (val, key) {
						val.element.open = false;
					});

...
define([
	...
	angular.module('socioApp.helperFactory', ['socioApp.user.userHelper'])
		.factory('helperFactory', function (localstorage, $location, httpServices, $timeout, $q) {
			 ...
			 */
			helper.convertTimeStampToUTC = function (millis) {
				return helper.millisToUTCDate(millis);
			}

			helper.millisToUTCDate = function (millis) {
				return helper.toUTCDate(new Date(millis));
			}

			helper.toUTCDate = function (date) {
			...
			/**
			 * @method get utc timestamp from date string
			 */

			helper.getUTCTimeStamp = function (dateString) {
			 ...
			 * @method convert  time Stamp to date 
			 */
			helper.formatUTCDate = function (dateString, format = 'dd/MM/yyyy') {
				if (typeof dateString != 'undefined') {
					var date = new Date();
					var addLocalTimeStamp = date.getTimezoneOffset()*60*1000;
					var d = new Date(dateString - addLocalTimeStamp);
					var month = ("0" + (d.getMonth() + 1)).slice(-2)
					var day = ("0" + d.getDate()).slice(-2);
					var year = d.getFullYear();
					var sep = '/';
			...
			/**
			 * method to find all words with a special character from an 
			 * input string
			 */
			helper.getAllWords = function (strFromSearch, re = false) {
				if (!re) {
					re = /(?:^|[ ])#([a-zA-Z]+)/gm;
				}
				var m;
				var setWords = [];
				while ((m = re.exec(strFromSearch)) != null) {
					console.log('m.index: ', m.index);
					console.log('re.lastIndex: ', re.lastIndexx);
					if (m.index === re.lastIndex) {
						re.lastIndex++;
					}
					setWords.push(m[0].trim());
					/*console.log(m)
		...
		.factory('menuFactory', function (localstorage, userHelper, rolesHelper, helperFactory, $templateCache, $location, $timeout) {
			...
			menuFactory.checkMenuActive = function (menuItems) {
				...
				angular.forEach(menuItems, function (menu) {
					...
					angular.forEach(menu.children, function (child) {
						if (child.href === locPath) {
							if (locPath == '/summary' || locPath == '/composePost') {
								child.active = true;
								matchUrlFound = true;
			...
			menuFactory.getMenuItems = function ($scope) {
				...
				if (userAccessLevel == 'super_admin') {
					settingsMenu.children.push(clientListMenu)
				}

				if (userAccessLevel == 'client_admin') {
					settingsMenu.children.push(clientViewMenu)
				}


				var _showBrandMenu = rolesHelper.canCreate('MANAGE_BRAND');
				if (_showBrandMenu) {
					settingsMenu.children.push(brandListMenu);
				}
				...
				if (_showUserList) {
					settingsMenu.children.push(userListMenu);
				}
				...
				if (_showTeamList) {
					settingsMenu.children.push(teamListMenu);
				}
				...
				if (rolesMenuAccess && rolesMenuAccess.permissions.indexOf('GET') > -1) {
					...
					settingsMenu.children.push(roleListMenu);
				}
				vm.menu = null;
				vm.msg = null;
